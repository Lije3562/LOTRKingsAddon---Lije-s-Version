...
public final class AquaModelBipedLogic {
    ...
    public static void pre(ModelBiped m,float limb,float amount,float age,float yaw,float pitch,float scale,Entity e) {
        if(FirstPersonArmRenderContext.isActive()) m.isSneak=false; else if(e instanceof EntityPlayer&&e instanceof IPlayerResizeable)m.isSneak=e.isSneaking()||((IPlayerResizeable)e).getPose()==Pose.CROUCHING;
        if(!ConfigHandler.MiscellaneousConfig.eatingAnimation||!(e instanceof EntityLivingBase))return; EntityLivingBase living=(EntityLivingBase)e;
    ...
    public static void living(ModelBiped m,EntityLivingBase e,float a,float b,float partial){if(e instanceof IPlayerResizeable)((IModelBipedSwimming)m).setSwimAnimation(((IPlayerResizeable)e).getSwimAnimation(partial));}
