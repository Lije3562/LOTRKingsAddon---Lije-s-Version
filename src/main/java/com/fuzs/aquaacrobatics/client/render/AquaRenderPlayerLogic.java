...
public final class AquaRenderPlayerLogic { private AquaRenderPlayerLogic(){}
 ...
 public static double crouchingY(AbstractClientPlayer p,double original){return ((IPlayerResizeable)p).getPose()!=Pose.CROUCHING?original:original+AquaPlayerRenderLogic.getCrouchingRenderY(p.ySize,p.isSneaking()&&!(p instanceof EntityPlayerSP));}
 public static void rotations(AbstractClientPlayer p,float a,float yaw,float partial){if(!EFRIntegration.isElytraFlying(p)){float f=((IPlayerResizeable)p).getSwimAnimation(partial);float target=p.isInWater()?-90F-p.rotationPitch:-90F;GL11.glRotatef(MathHelperNew.lerp(f,0,target),1,0,0);if(((IPlayerResizeable)p).isActuallySwimming())GL11.glTranslatef(0,-1,.3F);}}
}
